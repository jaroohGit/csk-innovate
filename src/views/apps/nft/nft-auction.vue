...
export default {
    ...
    mounted() {
        ...
        var countDownDates = setEndDates.map(date => this.startCountDownDate(date));

        const startCountdown = (elementId, countDownDate) => {
            const intervalId = setInterval(() => {
                ...
                const el = document.getElementById(elementId);
                if (el) {
                    el.textContent = `${format(days)} : ${format(hours)} : ${format(minutes)} : ${format(seconds)}`;
                } else {
                    clearInterval(intervalId);
                }
            }, 1000);
        ...
        countDownDates.forEach((date, index) => {
            const elementId = `auction-time-${index + 1}`;
            if (document.getElementById(elementId)) {
                startCountdown(elementId, date);
            }
        });
